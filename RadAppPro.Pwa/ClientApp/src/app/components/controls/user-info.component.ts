...
export class UserInfoComponent implements OnInit {
  ...
  private onCurrentUserDataLoadSuccessful(user: User, roles: Role[]) {
    ...
    let verificationEmailSent = this.localStorage.getDataObject<boolean>(this.getDBkey_VERIFICATION_EMAIL_SENT(this.user.id));

    if (!verificationEmailSent && !this.emailConfirmed) {
      let sendVerificationEmail_WindowsFuncName = "userinfo_sendVerificationEmail";
      window[sendVerificationEmail_WindowsFuncName] = this.sendVerificationEmail.bind(this);

      let confirmEmailMsg = `Your account email has not been verified. <a href="javascript:;" onclick="window.${sendVerificationEmail_WindowsFuncName}()">Click here to resend verification email</a>`;
