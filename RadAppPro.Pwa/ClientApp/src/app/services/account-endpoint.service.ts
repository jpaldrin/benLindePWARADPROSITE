...
export class AccountEndpoint extends EndpointFactory {
  ...
  getNewUserEndpoint<T>(userObject: any, isPublicRegistration?: boolean): Observable<T> {
    ...
    let endpointUrl = isPublicRegistration ? this.usersPublicUrl : this.usersUrl;

    return this.http.post<T>(endpointUrl, JSON.stringify(userObject), this.getRequestHeaders()).pipe<T>(
      catchError(error => {
  ...
  getSendConfirmEmailEndpoint<T>(): Observable<T> {
    let endpointUrl = this.sendConfirmEmailUrl;
  ...
  getConfirmUserAccountEndpoint<T>(userId: string, confirmationCode: string): Observable<T> {
    let endpointUrl = `${this.confirmEmailUrl}?userid=${userId}&code=${confirmationCode}`;
  ...
  getRecoverPasswordEndpoint<T>(usernameOrEmail: string): Observable<T> {
    let endpointUrl = this.recoverPasswordUrl;

    return this.http.post<T>(endpointUrl, JSON.stringify({ usernameOrEmail: usernameOrEmail }), this.getRequestHeaders()).pipe<T>(
      catchError(error => {
  ...
  getResetPasswordEndpoint<T>(usernameOrEmail: string, newPassword: string, resetCode: string): Observable<T> {
    ...
    return this.http.post<T>(endpointUrl, JSON.stringify({ usernameOrEmail: usernameOrEmail, password: newPassword, resetcode: resetCode }), this.getRequestHeaders()).pipe<T>(
      catchError(error => {
